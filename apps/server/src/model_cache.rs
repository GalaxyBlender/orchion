use crate::config::ModelRegistrySection;
...
pub async fn ensure_available_models<M: ModelSpec>(
    ...
    for model in models {
        tracing::info!(model = ?model, models_dir = %dir.display(), "ensuring {label} model is available");
        let path = downloader.download(*model, dir).await?;
        tracing::info!(model = ?model, path = %path.display(), "{label} model cache ready");
    }
    Ok(())
}
