...
export const AppLayout: React.FC = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Page shortcuts: Cmd/Ctrl + 1/2/3/4/5/6/7
      if ((e.metaKey || e.ctrlKey) && ["1", "2", "3", "4", "5", "6", "7"].includes(e.key)) {
        e.preventDefault();
        if (e.key === "1") navigate("/llm");
        if (e.key === "2") navigate("/asr");
        if (e.key === "3") navigate("/tts");
        if (e.key === "4") navigate("/ocr");
        if (e.key === "5") navigate("/pdf");
        if (e.key === "6") navigate("/models");
        if (e.key === "7") navigate("/settings");
      }
    };
