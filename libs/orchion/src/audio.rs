...
impl AudioOutputFormat {
    ...
    fn ffmpeg_output_args(self) -> Option<&'static [&'static str]> {
        match self {
            Self::Wav | Self::Pcm => None,
            Self::Mp3 => Some(&["-acodec", "libmp3lame", "-f", "mp3"]),
            Self::Aac => Some(&["-acodec", "aac", "-f", "adts"]),
            Self::Opus => Some(&["-acodec", "libopus", "-f", "ogg"]),
            Self::Flac => Some(&["-acodec", "flac", "-f", "flac"]),
        }
    }
}
...
impl FfmpegAudioCodec {
    ...
    pub async fn decode_for_asr(&self, input: Vec<u8>) -> Result<DecodedAudio> {
        ...
        crate::blocking::run(move || decode_for_asr_blocking(&binary, input)).await
    }
    ...
    pub async fn encode_tts_samples(
        ...
        let binary = self.binary.clone();
        crate::blocking::run(move || encode_tts_blocking(&binary, samples, sample_rate, format))
            .await
    }
}
...
fn encode_tts_blocking(
    ...
    if format == AudioOutputFormat::Pcm {
    ...
    let Some(output_args) = format.ffmpeg_output_args() else {
        return Err(OrchionError::InvalidAudio {
            reason: format!("unsupported ffmpeg output format `{format}`"),
        });
    };
    ...
    args.extend(output_args.iter().map(|arg| (*arg).to_string()));
...
#[cfg(feature = "tts")]
fn encode_wav_bytes(samples: &[f32], sample_rate: u32) -> Result<EncodedAudio> {
    ...
    let mut cursor = Cursor::new(Vec::new());
    {
        let mut writer = hound::WavWriter::new(&mut cursor, spec).map_err(|error| {
            OrchionError::InvalidAudio {
                reason: format!("failed to create WAV encoder: {error}"),
            }
        })?;
